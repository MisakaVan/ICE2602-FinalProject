...
class HtmlIndexer:
    def __init__(self,
        ...
        print(similarity_class)

        # New in lab 6
        config.setSimilarity(similarity_class)
    ...
    def run(self):
        ...
        for _filename, url in self.html_dict.items():
            ...
            logging.info(f"Processing {_r_filename}")
            try:
                ...
                try:
                    ...
                    logging.warning(f"File {_filename} has no title")
                    title = "<Missing Title>"
                ...
                logging.info(f"{_filename} has content size {len(text)}")

                doc.add(Field("contents", text, t2))

                self.writer.addDocument(doc)
            except Exception as e:
                logging.warning(f"Failed processing {_filename}: {e}")
                # raise e
